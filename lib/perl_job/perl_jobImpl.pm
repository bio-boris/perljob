...
sub new
{
    ...
    my $scratch = $cfg->val('perl_job', 'scratch');
...
=head2 run_perl_job
  ...
  $output = $obj->run_perl_job($params)
...
$output is a perl_job.ReportResults
...
sub run_perl_job
{
    ...
    if (@_bad_arguments) {
	my $msg = "Invalid arguments passed to run_perl_job:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
							       method_name => 'run_perl_job');
    }

    my $ctx = $perl_job::perl_jobServer::CallContext;
    my($output);
    #BEGIN run_perl_job
    ...
    if (@_bad_returns) {
	my $msg = "Invalid returns passed to run_perl_job:\n" . join("", map { "\t$_\n" } @_bad_returns);
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
							       method_name => 'run_perl_job');
    }
    return($output);
