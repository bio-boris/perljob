package perl_job::perl_jobServer;
...
sub call_method {
    ...
    my $ctx = perl_job::perl_jobServerContext->new($self->{loggers}->{userlog},
...
{
    # Service perl_job requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for perl_job but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = perl_job::perl_jobServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service perl_job.");
    }
...
package perl_job::perl_jobServerContext;

use strict;

=head1 NAME

perl_job::perl_jobServerContext
...
package perl_job::perl_jobServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = perl_job::perl_jobServer->new(
