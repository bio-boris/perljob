...
my $config = new Config::Simple($config_file)->get_block('perl_job');
...
$perl_job::perl_jobServer::CallContext = $ctx;
my $impl = new perl_job::perl_jobImpl();

sub get_ws_name {
    if (!defined($ws_name)) {
        my $suffix = int(time * 1000);
        $ws_name = 'test_perl_job_' . $suffix;
...
eval {
    ...
    my $ret = $impl->run_perl_job({workspace_name => get_ws_name(),
...

{
    ...
    sub provenance {
        my($self) = @_;
        return [{'service' => 'perl_job', 'method' => 'please_never_use_it_in_production', 'method_params' => []}];
    }
    sub authenticated {
        return 1;
    }
